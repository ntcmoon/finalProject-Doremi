    ...
    <>
      <div className="w-screen">
        <div className="container m-auto relative">
          <h2 className="text-center text-6xl font-bold p-16 text-amber-400">Matching Game</h2>
          <div className="grid grid-cols-10 items-center gap-4">
            {shuffleCards.map((d) => (
              <div key={d.id} onClick={() => selectCard(d)} style={{ opacity: pickedCards.includes(d.id) ? 0.5 : 1, transition: 'opacity 0.3s' }} >
                <img src={pickcard.map((c) => (c.id)).includes(d.id) ? (d.url) : (d.img)} width="120px" height="120px"  />
              </div>
            ))}
          </div>
          ...
          <div className="float-right  text-6xl text-yellow-400 font-bold tracking-wider ">{score} / 10</div>
          {win && (
          <button
